...
function [ lam,t2,et,alphat ] = DWX( T,t2,et,k,e,alpha,mu,kP,ReT,Pr,Pl,mesh,iter,RadMod)
    ...
    Cr2  = kP(1:end-1)./cr22.*atan(cr22./kP(1:end-1));
    ...
    if RadMod == 1
        R      = 0.5*(t2./(et + cr11*t2.*(Cr1).*(1-Cr2).*kP(1)).*e./k); 
    else
        R      = 0.5*(t2./et.*e./k); 
    end
