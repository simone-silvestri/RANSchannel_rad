function plotcases(cas,gry,ReT,var,cha)
...
for c=1:length(cas)
    ...
    if cha==0 || cha==3 || cha==4
        plot(dns(c).m(1:sk:end,1),dns(c).m(1:sk:end,5),'o','Color',DNS,'LineWidth',width);
        ylim([0 1.0]);
        xticks([0.5 1 1.5])
        yticks([0.2 0.4 0.6 0.8])
        grid on;
    elseif cha==1
        plot(dns(c).m(1:sk:end/2,1),1-dns(c).m(1:sk:end/2,5),'o','Color',hot,'LineWidth',width);
...
%     string = strcat('solution/V2F/PRT/',cas{c});
%     v2f.prt = importdata(string);
%     string = strcat('solution/V2F/PRT/',cas{c},'_rad_0');
%     v2f.prr = importdata(string);
    y = sa.no(:,1);
    
    if(gry==1)
        string = strcat('solution/V2F/DWX/',cas{c},'_rad_2');
        v2f.dwrng = importdata(string);
        string = strcat('solution/V2F/V2T/',cas{c},'_rad_2');
        v2f.v2rng = importdata(string);
    end
    ...
    elseif cha == 3
        ...
        if(gry==1)
            plot(y(1:end),v2f.v2rng(1:end,3),':','Color',enhanced);
        end
%     elseif cha == 4
%         %plot(y(1:end),sa.no(1:end,3),':','Color',grey2);
%         plot(y(1:end),v2f.no(1:end,3),'--','Color',v2fno);
%         plot(y(1:end),v2f.prt(1:end,3),'-.','Color',v2fdx);
%         plot(y(1:end),v2f.prr(1:end,3),'-','Color',v2fdr);
%         if(gry==1)
%             plot(y(1:end),v2f.dwrng(1:end,3),':','Color',enhanced);
%         end
    else
    ...
    if(var>=2)
        ...
        elseif cha==3
            ...
            if(gry==1)
                plot(y(1:end),v2f.v2rng(1:end,7),':','Color',enhanced);
            end 
        else
    ...
    if(var>=3)
        ...
        if cha==0
            ...
            maxfig(3,c) = max([sa.no(:,6).*sa.no(:,13);v2f.no(:,6).*v2f.no(:,13);v2f.dwx(:,6).*v2f.dwx(:,13);v2f.dwr(:,6).*v2f.dwr(:,13)]);
        elseif cha==3
            %plot(y(1:end),sa.no(1:end,6).*sa.no(1:end,13),':','Color',grey2);
            plot(y(1:end),v2f.no(1:end,6).*v2f.no(1:end,13),'--','Color',v2fno);
            plot(y(1:end),v2f.v2t(1:end,6).*v2f.v2t(1:end,13),'-.','Color',v2fdx);
            plot(y(1:end),v2f.v2r(1:end,6).*v2f.v2r(1:end,13),'-','Color',v2fdr);
            if(gry==1)
                plot(y(1:end),v2f.dwrng(1:end,6).*v2f.v2rng(1:end,13),':','Color',enhanced);
            end
            maxfig(3,c) = max([v2f.no(:,6).*v2f.no(:,13);v2f.v2t(:,6).*v2f.v2t(:,13);v2f.v2r(:,6).*v2f.v2r(:,13)]);
        else
